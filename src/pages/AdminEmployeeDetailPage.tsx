...
const AdminEmployeeDetailPage = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editRole !== currentRole) {
        const roleResponse = await supabase.functions.invoke("admin-manage", {
          body: {
            action: "assign-role",
  ...
  // Determine if manager dropdown should show based on selected role
  const showManagerSelect = editRole === "employee" || editRole === "team_lead" || editRole === "hr";

  // Filter managers: only show users with manager/admin role, exclude self
