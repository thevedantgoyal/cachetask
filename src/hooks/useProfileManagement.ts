...
export const useUpdateProfile = () => {
  ...
  return useMutation({
    mutationFn: async (updates: Partial<ExtendedProfile>) => {
      ...
      const { id, user_id, email, ...safeUpdates } = updates as any;
      const { data, error } = await supabase
        .from("profiles")
        .update(safeUpdates)
        .eq("user_id", user.id)
...
export const useCompleteProfile = () => {
  ...
  return useMutation({
    mutationFn: async (profileData: {
      ...
      if (profileData.other_social_links) updatePayload.other_social_links = profileData.other_social_links as any;
