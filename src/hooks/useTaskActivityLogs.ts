  ...
  return useMutation({
    mutationFn: async ({
      ...
      const { error } = await supabase.from("task_activity_logs").insert([{
        ...
        old_value: (oldValue || null) as unknown as import("@/integrations/supabase/types").Json,
        new_value: (newValue || null) as unknown as import("@/integrations/supabase/types").Json,
      }]);
