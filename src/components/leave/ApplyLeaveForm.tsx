...
import { ArrowLeft, CalendarIcon, Paperclip, FileText, AlertCircle } from "lucide-react";
...
export const ApplyLeaveForm = ({ balances, calculateDays, validate, onSubmit, onCancel }: Props) => {
  ...
  const handleSubmit = () => {
    if (fromDate && toDate) {
      onSubmit(leaveCode, fromDate, toDate, halfDay, reason, attachment);
    }
  };

  const handleMockAttach = () => {
    setAttachment("supporting_document.pdf");
  };
    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <Button variant="outline" size="sm" className="gap-2" onClick={handleMockAttach}>
